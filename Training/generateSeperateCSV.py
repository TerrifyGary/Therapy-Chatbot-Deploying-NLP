...
def genertateFeatureCSV(mode,trainPortion):
...
def createData(name,data,mode):
    if mode =='collectioncont':
        modename = 'collectioncount'
    else:
        modename = mode
    header = ['title', 'label']
    with open(f'{modename}_{name}.csv', 'w', encoding='UTF8') as f:
        writer = csv.writer(f)
...
def main():
    modes = ['commentcount','likecount','collectioncont']
    names = ['train','test']
    for m in modes:    
        data = genertateFeatureCSV(m, 0.75)
